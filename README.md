...
⚠️ Гусянки не могут вырабатывать больше чем 50% базовой выработки шахт. Это называется "хардкап" (hardcap).
...
Потребление энергии: 1 гусеничиник потребляет 5 энергии за каждые 10% для 0-100% режима и 10 энергии за каждые перегруженные 10% для 110-150% режима. Потребление энергии снижается Камерой Разрыва (роктальский домик).
...
Данная теха ФЖ не имеет какого-то капа, поэтому можно использовать всеми. Но с ней достаточно быстро можно достигнуть хардкапа гусеничников, поэтому для Коллектора в ней нет смысла.
